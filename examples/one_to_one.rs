...
fn main() {
    ...
    let handle = thread::spawn(move || {
        ...
        println!(
            "ProducerThread: Sending poison after publishing {} messages to ringbuffer",
            sent
        );
        loop {
        ...
        ring_buffer.read(
            |msg_type_id, src_buffer, src_index, _src_length| {
                ...
                if msg_type_id == 42 {
                    is_poison = true;
                    println!(
                        "ConsumerThread: Got Poison - msg_type_id: {} value: {}, received total {}",
    ...
    println!(
        "Time to receive {} messages {} milliseconds",
        times_called,
        elapsed.as_millis()
    );
